...
(function (Global, document) {
    ...
    Global.$$ = {
        ...
        syncDtTmEvents: function syncDtTmEvents(a) {
            a = _box(a);
            a.map(function (a) {
                return $$.copyEvents(document.getElementById(a), document.getElementById('time_' + a), ['onchange', 'onblur']);
            });
        },

        hideFormField: function hideFormField(a) {
            _tryCatch(getFormElementDOM(a).style.display = 'none');
            _tryCatch(getFormElementDOM('caption_' + a).style.display = 'none');
